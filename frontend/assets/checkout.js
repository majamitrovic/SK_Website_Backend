(function () {
    ...
    var appConfig = window.CHECKOUT_CONFIG || {};
    var apiBaseUrl = (appConfig.apiBaseUrl || 'http://localhost:8001/api').replace(/\/$/, '');
    var payment = null;
    ...
    function loadPaymentScript(src) {
        return new Promise(function (resolve, reject) {
            ...
            script.onerror = function () {
                reject(new Error('Payment.js did not load.'));
            };
    ...
    function loadCheckoutConfig() {
        setStatus('Loading checkout...', 'pending');

        return fetch(apiBaseUrl + '/config.php', { headers: { Accept: 'application/json' } })
            ...
            .then(function (payload) {
                window.ALLSECURE_CONFIG = {
                    publicIntegrationKey: payload.publicIntegrationKey,
                    missingPublicKey: !payload.publicIntegrationKey
                };

                form.elements.amount.value = payload.defaultAmount || form.elements.amount.value;
                form.elements.currency.value = payload.defaultCurrency || form.elements.currency.value;
                form.elements.description.value = payload.defaultDescription || form.elements.description.value;

                if (document.getElementById('recurring_amount')) {
                    document.getElementById('recurring_amount').value = payload.defaultAmount || form.elements.amount.value;
                }

                if (document.getElementById('recurring_start_datetime')) {
                    document.getElementById('recurring_start_datetime').value = payload.recurringStart || '';
                }

                return loadPaymentScript(payload.paymentJsUrl);
            });
    }
    ...
    function handleGatewayResponse(payload) {
        ...
        if (result.returnType === 'FINISHED' && result.success) {
                window.location.href = '/success.html?merchant_transaction_id=' + encodeURIComponent(payload.merchantTransactionId);
                return;
        }
    ...
    loadCheckoutConfig()
        .then(initializePaymentJs)
        .catch(function (error) {
            setBusy(false);
            setStatus(error.message || 'Checkout configuration failed to load.', 'error');
        });
}());
